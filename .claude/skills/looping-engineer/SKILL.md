...
Keep Claude as the owner. Read repository guidance, define observable success criteria for the user's requested deliverable, and identify the smallest useful task list.
...
Use relevant floorX skills for non-obvious domain constraints. Consult consult-codex or consult-antigravity only when a focused read-only review would materially help and consultation is available and authorized. Keep the active model; honor explicit model preferences. Do not create duplicate implementation owners.
