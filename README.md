...
A sales-floor digital twin built around one versioned floor document. The current implementation includes a framework-independent TypeScript model, a versioned fixture catalog, an interactive 2D floor editor and a read-only 3D viewer. API persistence and the broader planner toolset are later milestones.
