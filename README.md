...
Open http://127.0.0.1:3000 after starting `pnpm dev`. Drag a fixture card from the palette onto the floor, select and move it, use its handles to resize or rotate it, then edit its numeric properties if needed. Shift-click fixtures to select a group and drag it together. Copy, Paste and Duplicate are in the toolbar; Cmd/Ctrl+A, C, V and D provide shortcuts while the editor is focused. Keyboard users can focus a fixture card and press Enter or Space to place it at the view center. Use Undo/Redo for document edits and Cmd/Ctrl-drag to pan or Cmd/Ctrl-scroll to zoom. Save JSON downloads the layout as a file; Open JSON loads a saved file. Each save creates a new download, so keep the latest copy. Recover browser save can open layouts stored by the old Save locally action.
